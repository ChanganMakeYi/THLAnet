...
class Config:
    ...
    def __init__(self, args):
        ...
        self.val_split = 0.10
        self.classification_threshold = 0.05  # 降低阈值
...
def initialize_model(config):
    """初始化 MLP 模型并验证输出形状。"""
    try:
        model = mlp.MLPtrain(hidden_dims=[1024, 512, 256], dropout_rate=0.3).to(config.device)
        dummy_input = torch.randn(1, 80, 1280).to(config.device)
...
def evaluate(model, dataloader, config):
    ...
    # 动态阈值优化
    best_f1, best_threshold = metrics["f1"], config.classification_threshold
    for threshold in [0.01, 0.05, 0.1, 0.15]:
        predicted = (outputs_all > threshold).astype(float)
        f1_temp = f1_score(labels_all, predicted, zero_division=0)
        if f1_temp > best_f1:
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=128, help='批次大小')  # 增大批次
    parser.add_argument('--lr', type=float, default=0.001, help='学习率')
    parser.add_argument('--accum_steps', type=int, default=4, help='梯度累积步数')
    parser.add_argument('--neg_pos_ratio', type=float, default=1.0, help='负样本:正样本比例')  # 降低比例
